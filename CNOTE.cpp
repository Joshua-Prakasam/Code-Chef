...
Example case 1. In this case, Chef wants to write X = 3 pages long poetry, but his notebook has only Y = 1 page.
                ...
                The first notebook has P1 = 3 pages, but Chef cannot buy it, because its price is C1 = 4 rubles.
                The second notebook has P2 = 2 pages, and its price is C2 = 2 rubles.
...
int main(){
  ...
  while(runningTime--)
  {
    int reqPages, hasPage, moneyLeft, possibleNoteBookCount;
    char flag = 0;
    cin >> reqPages >> hasPage >> moneyLeft >> possibleNoteBookCount;
    reqPages -= hasPage;
    if(reqPages < 1)
    cout << "Lucky";
    else
    {
        while(possibleNoteBookCount--)
        {
            int page, cost;
            cin >> page >> cost;
            if (cost <= moneyLeft && page >= reqPages)
            flag = 1;
        }
        if(flag)
        {
